...
def sa_pgd(model, X, y, alpha, restarts, norm):
    max_loss = torch.zeros(y.shape[0]).cuda()
    max_delta = torch.zeros_like(X).cuda()
    for _ in range(restarts):
        delta = torch.zeros_like(X).cuda()
        if norm == "l_inf":
            delta.uniform_(-2/255., 2/255.)
        elif norm == "l_2":
            delta.normal_()
            d_flat = delta.view(delta.size(0),-1)
            n = d_flat.norm(p=2,dim=1).view(delta.size(0),1,1,1)
...
def get_args():
    ...
    parser.add_argument('--fname', default='cifar_model_baseline_AWP', type=str)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_batches):
            ...
            if args.attack == 'pgd':
                ...
                #delta = sa_pgd(model, X, y, 1/255., args.restarts, args.norm)
                delta = delta.detach()
            # Natural training
        ...
        if not args.eval:
            logger.info('%d \t %.1f \t \t %.1f \t \t %.4f \t %.4f \t %.4f \t %.4f \t \t %.4f \t \t %.4f \t %.4f \t\t %.4f \t %.4f \t\t %.4f \t %.4f',
